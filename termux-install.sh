...
if [[ -d "Kyran-Pyro" ]]; then
  cd Kyran-Pyro
elif [[ -f ".env.dist" ]] && [[ -f "main.py" ]] && [[ -d "modules" ]]; then
  :
else
  git clone https://github.com/Kyran-Pyro/Kyran-Pyro || exit 2
  cd Kyran-Pyro || exit 2
fi

if [[ -f ".env" ]] && [[ -f "kyran.session" ]]; then
  echo "It seems that Kyran-Pyro is already installed. Exiting..."
  exit
fi
...
if [[ $api_id = "" ]]; then
  api_id="22156937"
  api_hash="0f8f66b06b1c53b9263bcfb1123e9c85"
else
...
echo
echo "============================"
echo "Great! Kyran-Pyro installed successfully!"
