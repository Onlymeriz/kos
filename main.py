...
app = Client(
    ...
    device_model=f"Kyran-Pyro @ {gitrepo.head.commit.hexsha[:7]}",
...
async def main():
    ...
    try:
        ...
        if str(e) == "database is locked" and os.name == "posix":
            ...
            subprocess.run(["fuser", "-k", "kyran.session"])
            restart()
        raise
    except (errors.NotAcceptable, errors.Unauthorized) as e:
        logging.error(
            f"{e.__class__.__name__}: {e}\n"
            f"Moving session file to kyran.session-old..."
        )
        os.rename("./kyran.session", "./kyran.session-old")
        restart()
    ...
    logging.info("Kyran-Pyro started!")
